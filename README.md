...
A Entrada (Mundo Natural): O usuário digita um texto (ex: "O que é IA Generativa?").

Tokenização BPE (O Fatiador): O texto voa pela interface e é fragmentado em sub-palavras (tokens).

Mapeamento de Vocabulário: As palavras "giram" e se transformam em IDs numéricos puros.

Matriz de Embeddings: Cada ID busca sua assinatura semântica multivetorial, normalizada em uma matriz bidimensional gerada em tempo real.

Espaço Latente e Atenção (Mundo 3D): Os vetores são projetados em um ambiente 3D simulado com Three.js. Os nós orbitam e se conectam (simulando a relação Query-Key-Value do mecanismo de autoatenção).

Decodificação Auto-Regressiva: O modelo calcula as probabilidades (Logits), projeta os novos IDs e os reverte em linguagem humana (a resposta).
...
Crie uma Branch para sua modificação (git checkout -b feature/NovaAnimacaoDidatica).

Faça o Commit de suas alterações (git commit -m 'Add: nova explicação sobre Softmax').

Faça o Push para a Branch (git push origin feature/NovaAnimacaoDidatica).

Abra um Pull Request.

📄 Licença

Este projeto está sob a licença MIT. Consulte o arquivo LICENSE para mais detalhes. O uso em salas de aula, palestras e workshops é não apenas permitido, mas fortemente encorajado! 👨‍🏫👩‍🏫
